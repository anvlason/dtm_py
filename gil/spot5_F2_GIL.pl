...
#chomp($JOBNAME);
  ...
  my $BINDIR = 'c:\app\BIN_IC64';

foreach $DIR(glob("D:\\GIL\\FS2")){
 if(-d $DIR){
  my $JOBNAME = GetName($DIR);
  print("$JOBNAME\n");
  my $INDIR = "D:\\GIL\\$JOBNAME";
  my $DONEDIR = "$INDIR\\DONE"; mkdir($DONEDIR);
  my $OUTDIR = "E:\\GIL"; mkdir($OUTDIR);
  $OUTDIR="$OUTDIR\\FS2"; mkdir($OUTDIR);
  foreach $pan(glob("$INDIR\\FS2_*_PAN\\FS2_*_PAN")){
   if(-d $pan){
	...
	system ("xcopy /Y $pan\\metadata.dim $OUTDIR\\$outname");
