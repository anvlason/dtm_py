...
my $INDIR="G:\\Departments\\production\\plproduction\\GIL\\SPOT67\\KKX15-16\\FULLSET";
...
while (my $ln = <LST>){
    ...
    foreach $dim(glob("$INDIR\\$prod_name\\VOL_SPOT*\\IMG_SPOT*_P*\\DIM*.xml")){
           ...
           system("test_html_v3.pl $OUTDIR\\$reg_name\\$odir ORTHO");
