my $INDIR="G:\\Departments\\production\\plproduction\\SPOT6_OUT\\THR1";
my $DONEDIR = "$INDIR\\DONE"; mkdir($DONEDIR);
my $QLDIR = "G:\\Departments\\production\\plproduction\\SPOT6_OUT\\QL";
my $BINDIR="C:\\app\\bin";
#my $METPATH="G:\\MET";
while(1){
 if(-f "$INDIR\\stop.vec") { exit; }
 foreach $dir(glob("$INDIR\\DS_SPOT*_*")){
  if(!-f "$dir\\done") { next; }
