...
def fitPos(pos,model):
    fit_x = interpolate.interp1d(pos[:,3],pos[:,0],kind=model,bounds_error=False)
    fit_y = interpolate.interp1d(pos[:,3],pos[:,1],kind=model,bounds_error=False)
    fit_z = interpolate.interp1d(pos[:,3],pos[:,2],kind=model,bounds_error=False)
...
def fitPosS(pos,order):
    fit_x = interpolate.InterpolatedUnivariateSpline(pos[:,3],pos[:,0],k=order)
    fit_y = interpolate.InterpolatedUnivariateSpline(pos[:,3],pos[:,1],k=order)
    fit_z = interpolate.InterpolatedUnivariateSpline(pos[:,3],pos[:,2],k=order)
    return (fit_x,fit_y,fit_z)
    #return (fit_x(jd)[0],fit_y(jd)[0],fit_z(jd)[0])


name = sys.argv[1]#"I:\\ABS_CHK\\RU\\IAP_SPOT6_201509080537027_RU1_RU1_RU1_RU1_00000189_396_00000_03169_E081N55.xml"
imname = sys.argv[2]#"I:\\ABS_CHK\\L1\\DS_SPOT6_201509080537027_RU1_RU1_RU1_RU1_E081N55_03170\\CAM2_RETB_PAN.tif"
ticname = re.sub(".tif",".tic",imname,flags=re.IGNORECASE)
ggname = re.sub(".tif",".gg0.ascii",imname,flags=re.IGNORECASE)

cam = int(re.findall('CAM([1-2]{1})_',imname)[0])
ret = re.findall('_RET([A-B]{1})',imname)[0]
...
RetinaPar = readIAPretina(name,'Camera_%d'%(icam),'Retina_%s'%(ret))[0] #JD0,LP,first_line,start_line,last_line


y_shift = 0#-0.5#-0.5
ovl = 0#-121#0
h = 500.0
step = 32#int(137*scale)
interp = 'cubic'
usegg = 1
usetic = 0
IAP_Quat = readIAPquat(name)
IAP_Pos = readIAPpos(name)
#interpolate quaternions and position using source IAP data (none smoothed)
order = 3
#fit_q = fitQuat(IAP_Quat,interp)
#fit_p = fitPos(IAP_Pos,interp)
fit_q = fitQuatS(IAP_Quat,order)
fit_p = fitPosS(IAP_Pos,order)
#calculate psi
psi = getdir_rn_all(pr_width+step,x_shift,tanpsix,tanpsiy)
#calculate coordinates
#retina
#loop for Y from first valid line
#write header
if usetic == 1:
    tic = open(ticname,'w')
    ...
    tic.write("prjt=proj\n")
    height = 27244
    #for y in (range(int(RetinaPar[3]),int(RetinaPar[4]),step)):
    i = 0
    for y in (range(1,height+step,step)):
    #for y in (range(int(RetinaPar[2]),int(RetinaPar[4])+step*10,step)):
        t = getTbyline(y+line_shift,RetinaPar[0],RetinaPar[1])
        ...
        mat = quat2matrix(q[0])
        llstr = ''
        for x in range(1,pr_width+step,step):
            sdir = [psi[0,x],psi[1,x],psi[2,x]]#getdir_rn(x,5,tanpsix,tanpsiy)
            sdir = rotate(sdir,mat_cam)
...
if usegg == 1:
    imsize = [int(RetinaPar[4]),6992]
#write header    
    gg = open(ggname,'w')
    gg.write("nSamples    %d\n"%(len(range(0,imsize[1]+step-1,step))))
    gg.write("nLines      %d\n"%(len(range(0,imsize[0]+step-1,step))))
...
#    gg.write("y1          %d\n"%(imsize[0]))
    gg.write("x0          %d\n"%(0))
    gg.write("y0          %d\n"%(int(RetinaPar[3]-1-50)))
    gg.write("x1          %d\n"%(imsize[1]+ovl))
    gg.write("y1          %d\n"%(int(RetinaPar[4]-1+50)))
    gg.write("separate    %d\n"%(0))
    gg.write("delimiter   %d\n"%(32))
    gg.write("points\n")
#write points
    i = 0
    for y in (range(1,int(RetinaPar[4])+step,step)):
#    for y in (range(1,imsize[0]+step,step)):
        t = getTbyline(y+line_shift,RetinaPar[0],RetinaPar[1])
        q = np.zeros((1,4),dtype=np.float_)
        p = np.zeros((1,3),dtype=np.float_)
        q[0,0] = np.float_(fit_q[0](t))
